...
The team-lead skill encodes the full nine-phase lifecycle: Intake → Research → Spec → Plan → CHECKPOINT → Build → Verify → Review → Ship. It runs in this Fable 5 session and dispatches `sonnet-developer`, `opus-developer`, or `fable-developer` (routed per task by the plan's `complexity` flag) plus Haiku web-researchers under strict 1-level dispatch using reference-based prompts.
